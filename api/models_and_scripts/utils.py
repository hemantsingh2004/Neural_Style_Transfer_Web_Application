...
import os
from models_and_scripts.model.definitions import Vgg19
...
def generate_out_img_name(config):
    ...
    if 'reconstruct_script' in config:
        suffix = f'_o_adam_h_{str(config["height"])}_m_{config["model"]}{config["img_format"][1]}'
    else:
        suffix = f'_o_adam_i_{config["init_method"]}_h_{str(config["height"])}_m_{config["model"]}_cw_{config["content_weight"]}_sw_{config["style_weight"]}_tv_{config["tv_weight"]}{config["img_format"][1]}'
    return prefix + suffix


def save_img(optimizing_img, dump_path, config, img_id, num_of_iterations):
