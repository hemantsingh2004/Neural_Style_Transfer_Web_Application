...
class Vgg19(torch.nn.Module):
    def __init__(self, requires_grad=False, use_relu=True):
        ...
        weights_path = os.path.join(os.path.dirname(__file__), './weights/vgg19-weights.pth')
        # Validate the existence of the weights file
        if not os.path.exists(weights_path):
            raise FileNotFoundError(f"VGG19 weights file not found at {weights_path}")

        model.load_state_dict(torch.load(weights_path, weights_only=True))
        # model.load_state_dict(torch.load('./weights/vgg19-weights.pth', weights_only=True))
